    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-lg font-semibold text-sky">{plot.title}</h3>
          {headerText ? (
      ...
      {plot.caveats.length > 0 ? (
        <ul className="mt-4 grid gap-2 text-sm text-graphite">
          {plot.caveats.slice(0, 3).map((caveat) => (
            <li key={caveat} className="rounded-md bg-[#f2f5ff] px-3 py-2">
              {caveat}
            </li>
          ))}
        </ul>
        ...
        <table className="min-w-full divide-y divide-ink/10 text-sm">
          ...
          <tbody className="divide-y divide-ink/8 bg-white">
            {visibleRows.map((row, rowIndex) => (
              <tr key={rowIndex} className="transition hover:bg-[#f2f5ff]">
                {plot.columns.map((column) => (
