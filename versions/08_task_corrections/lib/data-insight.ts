...
export function buildLocalInsight(dataset: SheetDatasetProfile | undefined) {
  if (!dataset) {
    return "В загруженном файле не найден надежный табличный набор данных, поэтому сейчас возможен только осторожный обзор без сильных выводов. Для более точного инсайта нужен файл с заголовками и несколькими заполненными строками. Если данные текстовые, лучше читать краткое резюме и задавать вопросы в чате.";
  }
  ...
  const shapeText = (shape?.claim ?? `Таблица содержит ${formatDatasetShape(context.rowCount, context.columnCount)}.`).replace(/[.!?…]+$/u, "");
  const structureSentence = [
    shapeText,
    usefulColumns.length > 0
      ? `полезные поля для обзора: ${usefulColumns.join(", ")}`
      : "сильных аналитических полей немного",
  ].join("; ") + ".";
  const mainFinding = groupLeader?.claim ?? category?.claim ?? "Самое надежное наблюдение связано со структурой строк и распределением заполненных значений.";
  const numericText = numeric?.evidence?.replace(/[.!?…]+$/u, "");
  const warningText = warning?.claim?.replace(/[.!?…]+$/u, "");
  const caveat = numericText || warningText
    ? `${numericText ? `По числовым значениям видно: ${numericText}` : ""}${numericText && warningText ? "; " : ""}${warningText ? `важная оговорка: ${warningText}` : ""}.`
    : "Серьезных предупреждений качества в подготовленном профиле не найдено.";

  return [structureSentence, mainFinding, caveat].join(" ");
}
...
export function buildLocalTextInsight(preprocessing: TextPreprocessingResult | undefined) {
  if (!preprocessing || preprocessing.stats.charCount === 0) {
    return "Текст принят, но в нем не найдено содержимого для надежного резюме. Проверьте, что файл содержит обычный читаемый текст, а не только вложения или неподдерживаемый формат. После этого LLM сможет подготовить краткое резюме.";
  }
  ...
  return compactText
    ...
    : "Текст принят, но после очистки не осталось читаемых предложений для краткого резюме. Для надежного вывода нужен читаемый текстовый фрагмент.";
}
