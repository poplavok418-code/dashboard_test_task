import {
  ...
  classifyExtension,
  getFileExtension,
  shouldPreprocessDelimitedSheet,
  shouldPreprocessTextFile,
  shouldPreprocessWorkbook,
  shouldReadTextSample,
  type LoaderResult,
  type ValidationCheck,
} from "./accepted-inputs";
import { formatFileSize } from "./format-file-size";
import { preprocessDelimitedTable } from "./preprocess-delimited-table";
import { preprocessDocx } from "./preprocess-docx";
import { preprocessText, unsupportedDocumentPreprocessing } from "./preprocess-text";
import { preprocessWorkbook } from "./preprocess-workbook";
...
export async function validateFileInput(files: File[]): Promise<LoaderResult> {
  ...
  try {
    const preprocessing = await (async () => {
      if (shouldPreprocessDelimitedSheet(extension)) {
        return preprocessDelimitedTable(await file.text(), file.name, extension);
      }

      if (shouldPreprocessWorkbook(extension)) {
        return preprocessWorkbook(file);
      }

      if (shouldPreprocessTextFile(extension)) {
        return preprocessText(await file.text(), file.name);
      }

      if (extension === ".docx") {
        return preprocessDocx(file);
      }

      return unsupportedDocumentPreprocessing(file.name, extension);
    })();
    ...
    checks.push({
      ...
      detail:
        preprocessing.kind === "sheets"
          ? `Подготовлено наборов данных: ${preprocessing.datasets.length}.`
          : `Подготовлено фрагментов текста: ${preprocessing.chunks.length}.`,
    });
    ...
    checks.push({
      ...
      ok: false,
      label: "Предобработка выполнена",
      detail: "Не удалось подготовить данные к следующему шагу.",
    });
    ...
    return {
      ...
      error: {
        code: "read_failed",
        message: "Не удалось выполнить предобработку файла.",
        suggestion: "Проверьте, что файл не поврежден, и попробуйте экспортировать данные заново.",
      },
    };
  }
}
