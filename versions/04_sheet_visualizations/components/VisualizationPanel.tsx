...
function ChartFrame({ plot, children }: { plot: GraphPlot; children: ReactNode }) {
  return (
    <section
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wide text-sky">График по данным</p>
          <h3 className="mt-1 text-lg font-semibold text-ink">{plot.title}</h3>
          {plot.description ? <p className="mt-1 text-sm leading-6 text-graphite">{plot.description}</p> : null}
        </div>
        <span
...
export function VisualizationPanel({ plot }: { plot: GraphPlot }) {
  if (plot.kind === "empty") {
    return (
      <ChartFrame plot={plot}>
