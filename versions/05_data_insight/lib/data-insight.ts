...
export function buildLocalTextInsight(preprocessing: TextPreprocessingResult | undefined) {
  ...
  return [
    `Текст содержит примерно ${formatNumber(preprocessing.stats.wordCountApprox)} слов и ${formatNumber(preprocessing.stats.paragraphCount)} абзацев.`,
    "Для содержательного резюме нужен ответ LLM, потому что локальная предобработка не интерпретирует смысл текста.",
    `Подготовлено ${formatNumber(preprocessing.chunks.length)} текстовых фрагментов для анализа.`,
    preprocessing.warnings[0]?.message
      ? `Важная оговорка: ${preprocessing.warnings[0].message}`
      : "Критичных предупреждений предобработки для текста не найдено.",
    "Когда LLM-ответ доступен, здесь появится резюме ровно из пяти предложений.",
  ].join(" ");
}
