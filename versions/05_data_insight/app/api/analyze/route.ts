...
async function callAgent(prompt: string, userPayload: unknown) {
  ...
  if (!apiKey) {
    throw new Error("NEURALDEEP_API_KEY is not set.");
  }
  ...
  if (isTextInsight) {
    const insightResult = await runAgent(async () => {
      const response = await callAgent(insightPrompt, {
        locale: "ru",
        instruction: "Summarize the provided text in exactly five Russian sentences. Return the full required JSON object.",
        insightContext,
      });
