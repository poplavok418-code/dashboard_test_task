...
function validateRequest(payload: unknown): { ok: true; request: TalkToDataRequest } | { ok: false; status: number; code: string; message: string } {
  ...
  if (!candidate.insightContext || candidate.insightContext.sourceType !== "sheet") {
    return { ok: false, status: 400, code: "invalid_context", message: "Нужен компактный профиль табличных данных." };
  }

  if (!candidate.visualizationContext || candidate.visualizationContext.sourceType !== "sheet") {
