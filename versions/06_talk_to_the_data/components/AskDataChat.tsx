    ...
    <section data-testid="ask-data-panel" className="rounded-lg border border-ink/10 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-lg font-semibold text-ink">Спроси что-нибудь про эти данные</h3>
        </div>
      ...
      <div className="mt-4 grid gap-3">
        <textarea
          ...
          className="min-h-24 resize-none rounded-md border border-ink/12 bg-[#f8fafc] p-3 text-sm leading-6 text-ink outline-none transition placeholder:text-graphite/60 focus:border-saffron focus:ring-2 focus:ring-saffron/30"
        />
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            className="inline-flex h-10 items-center gap-2 rounded-md bg-ink px-4 text-sm font-semibold text-white transition hover:bg-sky focus:outline-none focus:ring-2 focus:ring-sky/35 disabled:cursor-wait disabled:opacity-70"
          >
      ...
      {state.notice ? (
        <p
          ...
          className={[
            ...
            state.status === "error" ? "border-red-200 bg-red-50 text-red-800" : "border-saffron/35 bg-[#fff7e8] text-ink",
          ].join(" ")}
        >
          {state.notice}
        </p>
      ...
      {state.message ? (
        <div data-testid="ask-data-answer" className="mt-4 rounded-md border border-ink/10 bg-[#f8fafc] p-4 text-sm leading-6 text-ink">
