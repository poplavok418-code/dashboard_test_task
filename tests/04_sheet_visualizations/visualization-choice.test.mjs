...
const cliPath = path.join(repoRoot, "versions/04_sheet_visualizations/index.mjs");
const promptPath = path.join(repoRoot, "prompts/visualization_agent_prompt.md");
const fixturesDir = path.join(repoRoot, "tests/04_sheet_visualizations/test_cases");
  ...
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [cliPath, "--file", path.join(fixturesDir, fileName), "--prompt", promptPath], {
      ...
      env: {
        ...
        LLM_MAX_TOKENS: "1200",
        LLM_TEMPERATURE: "0.2",
        NODE_PATH: path.join(repoRoot, "versions/04_sheet_visualizations/node_modules"),
      },
    });
    let stdout = "";
    let stderr = "";
